  ...
  return (
    <main className="min-h-screen bg-[var(--site-bg)] px-4 pb-20 pt-28 text-[var(--site-ink)] sm:px-6 lg:px-8">
      <article className="mx-auto max-w-3xl">
        <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--site-label)] transition hover:text-[var(--site-label-hover)]">
        ...
        <header className="mt-8">
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <span className="rounded-full border border-[var(--site-label-line)] px-3 py-1 font-medium text-[var(--site-label)]">{post.category}</span>
            <span className="inline-flex items-center gap-1 text-[var(--site-soft)]">
              <Clock className="h-3.5 w-3.5" />
              {post.readTime}
            </span>
          </div>
          <h1 className="mt-4 text-4xl font-semibold tracking-tight sm:text-5xl">{post.title}</h1>
          <p className="mt-5 text-base leading-relaxed text-[var(--site-muted)] sm:text-lg">{post.description}</p>
        ...
        <div className="mt-12 space-y-10">
          {post.sections.map((section, index) => (
            <section key={section.heading}>
              <p className="text-xs font-medium tabular-nums text-[var(--site-label)]">{String(index + 1).padStart(2, "0")}</p>
              <h2 className="mt-2 text-2xl font-semibold tracking-tight">{section.heading}</h2>
              <p className="mt-3 text-sm leading-relaxed text-[var(--site-muted)] sm:text-base">{section.body}</p>
            </section>
          ))}
        </div>
        ...
        <section className="mt-14 rounded-[1.6rem] border border-[var(--site-label-line)] bg-[var(--site-card)] p-6 sm:p-8">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <Link
              href="/services"
              className="inline-flex items-center justify-center gap-2 rounded-full bg-[#7CFFB2] px-6 py-3 text-sm font-semibold text-[var(--site-on-mint)] transition hover:bg-[#b6ffd4]"
            >
              View services
