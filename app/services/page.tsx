  ...
  return (
    ...
    <main className="min-h-screen bg-[#071018] px-4 pb-20 pt-28 text-[#f4f1ea] sm:px-6 lg:px-8">
      <section className="mx-auto max-w-6xl">
        <Link href="/" className="text-sm font-semibold text-[#7CFFB2] hover:text-[#b6ffd4]">
          Back to homepage
        </Link>
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {servicePages.map((service) => (
            <Link
              ...
              className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0c1826] p-6 transition hover:border-[#7CFFB2]/40"
              aria-label={`Open ${service.title} service page`}
            >
              <span className="absolute right-5 top-5 flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-[#7CFFB2]">
                <ArrowUpRight className="h-4 w-4" />
              </span>
              <h2 className="pr-12 text-2xl font-semibold tracking-tight">{service.title}</h2>
              <p className="mt-4 flex-1 text-sm leading-relaxed text-[#b7c3cf]">{service.description}</p>
        ...
        <section className="mt-14 rounded-[1.6rem] border border-[#7CFFB2]/25 bg-[#0c1826] p-6 sm:p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <a
              ...
              className="rounded-full bg-[#7CFFB2] px-6 py-3 text-sm font-semibold text-[#071018] transition hover:bg-[#b6ffd4]"
            >
              Book a Product Call
            </a>
            <a
              href="mailto:mnmemon549@gmail.com?subject=Service%20Inquiry"
              className="rounded-full border border-white/15 px-6 py-3 text-sm font-semibold transition hover:border-[#7CFFB2]/50 hover:text-[#7CFFB2]"
            >
              Ask About Services
            </a>
          </div>
